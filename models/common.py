...
class RepConv(nn.Module):
    ...
    def forward(self, inputs):
        ...
        #     return self.act(self.cbamc3(self.rbr_reparam(inputs)))
...
class RepConv_OREPA(nn.Module):
    ...
    def __init__(self, c1, c2, k=3, s=1, padding=1, dilation=1, groups=1, padding_mode='zeros', deploy=False, use_se=True, nonlinear=nn.SiLU()):
        ...
        if use_se:
            # self.se = SELayer(self.out_channels, self.out_channels // 16)
            self.se = CBAMC3(self.out_channels, self.out_channels)
            # self.se = CoordAtt(self.out_channels, self.out_channels)
...
class CBAMC3(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.spatial_attention = SpatialAttention(7) #空间注意力机制


        self.conv = nn.Conv2d(2*c2, 1, 3, padding=1, bias=False)
