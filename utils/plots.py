...
def plot_one_box(x, img, color=None, label=None, line_thickness=5):
    ...
    tl = line_thickness or round(0.005 * (img.shape[0] + img.shape[1])) + 4  # line/font thickness
    # tl = round(0.001 * (img.shape[0] + img.shape[1]))+4 # line/font thickness
    color = color or [random.randint(0, 255) for _ in range(3)]
    c1, c2 = (int(x[0]), int(x[1])), (int(x[2]), int(x[3]))
    cv2.rectangle(img, c1, c2, color, thickness=tl+1, lineType=cv2.LINE_AA)
    if label:
